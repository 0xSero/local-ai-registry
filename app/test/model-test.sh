#!/bin/bash
# The view model (core/Model.js): contrast tones on four themes, and every page built from snapshots of known shape
set -euo pipefail
ROOT=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")/.." && pwd)
command -v node >/dev/null || { echo "ok - the view model # SKIP node is not installed"; exit 0; }
ROOT=$ROOT node <<'JS'
const assert = (ok, what) => { if (!ok) { console.error('not ok - ' + what); process.exit(1) } console.log('ok - ' + what) }
const assertDeepEqual = (a, b, what) => {
  if (JSON.stringify(a) !== JSON.stringify(b)) { console.error('expected: ' + JSON.stringify(b) + '\nactual:   ' + JSON.stringify(a)); console.error('not ok - ' + what); process.exit(1) }
  console.log('ok - ' + what)
}
const model = require(process.env.ROOT + '/core/Model.js')
