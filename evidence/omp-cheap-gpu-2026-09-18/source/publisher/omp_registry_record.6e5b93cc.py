    ...
    for run in manifest["runs"]:
        ...
        if protocol_v2:
            ...
            status = ("accepted" if stat == "completed"
                      else "not-run" if stat in (None, "planned") else "infra-invalid")
        ...
        if status == "not-run":
            row["reason"] = "run not executed (sweep incomplete)"
            row["cause"] = "not_run"
...
def main(argv: list[str] | None = None) -> int:
    ...
    probe = load(artifacts / "probe.json")
    weights = load(artifacts / "weights.json")
    ...
    if not argv_engine:
        # A config-file engine (TabbyAPI) is configured by a mounted YAML. The old
        # generic asset-mount rewrite republished a plugin config that can DIFFER
        # from the image the run actually tested (it rewrote model_dir and forced a
        # /workspace/models mount even for images that bake their config into
        # /opt/models). Publishing a config other than the one tested is not honest
        # evidence, so the baked-config route is explicitly UNSUPPORTED until its
        # real implementation lands. The argv engines (SGLang/vLLM) carry their
        # whole configuration in the launch arguments and are fully supported.
        raise SystemExit(
            "config-file (TabbyAPI) publication is not implemented in this recorder; "
            "run the SGLang/vLLM argv path. Baked-config Tabby publishing awaits its "
            "real implementation and must not republish a possibly-divergent config."
        )
    config_text = ""
    ...
    if shipped is not None:
        ...
        "purpose": (
            ...
            "reasoning parsing enabled; byte-identical to the file the acceptance run used, "
            "with model_dir restored to the plugin's ${MODEL_ROOT} mount."
        ),
    ...
    write_json(registry / "speed-sweep" / f"{sweep_id}.json", {
        ...
        "measured_at": (acceptance.get("captured_at") or NOW)[:10],
        "accepted_at": (acceptance.get("captured_at") or NOW)[:10],
        "source": {
            "kind": "campaign",
        ...
        "metrics": {
            ...
            "latest_point_at": acceptance.get("captured_at") or NOW,
            ...
            "peak_prompt_tps": max(
                (row["prefill_tok_s"] for row in accepted if row["prefill_tok_s"]), default=None,
            ),
    ...
    capabilities = {name: bool(value) for name, value in acceptance["capabilities"].items()}
    ...
    if argv_engine:
        ...
        weights_mount = next(
            (mount for mount in launch.get("mounts") or []
             if mount.get("target") == "/workspace/models"),
            None,
        )
        if weights_mount is None:
            raise SystemExit("the candidate has no /workspace/models mount to bind the weights to")
        launch["mounts"] = [
            weights_mount,
    ...
    recipe["facts"] = {
        "capabilities.chat": {
            "state": "known", "reason": "omp_accept.py chat probe returned the pinned answer",
        ...
        "capabilities.reasoning": {
            ...
            "reason": "omp_accept.py reasoning probe: a separate reasoning_content phase "
                      "with the answer still in content",
            "provenance": run_provenance},
        "capabilities.tools": {
            ...
            "reason": "omp_accept.py tools probe: a forced call with valid arguments and a "
                      "round-tripped tool result",
            "provenance": run_provenance},
        "capabilities.vision": {
            "state": "known" if capabilities.get("vision") else "unknown",
            "reason": "omp_accept.py vision probe: solid red and blue PNGs named correctly",
            "provenance": run_provenance},
        "engine.version": {
    ...
    recipe["metadata"] = {
        **{key: value for key, value in recipe.get("metadata", {}).items()
           if key in ("weights_subdir",)},
        ...
        "acceptance": {
            "accepted_at": acceptance.get("captured_at"),
            "harness": "omp_accept.py (identity, chat, reasoning, tools, vision) + omp_bench_client.py",
            "served_model_id": plan["served_name"],
            "failed_probes": sorted(failures),
        },
        "measurements": {
            ...
            "sweep_rows": {
                ...
                "complete": expected_cells > 0 and len(accepted) == expected_cells,
            },
            "sweep_arms": [
                {
