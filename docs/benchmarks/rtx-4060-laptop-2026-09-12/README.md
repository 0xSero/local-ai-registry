...
The registry exporter, plugin gate and CLI preserve this explicit NVIDIA device declaration. Validation allows only `/dev/nvidia-uvm` for NVIDIA recipes rather than permitting arbitrary device passthrough. The plugin-side gate, launcher and download preflight are proposed separately in [omarchy-local-ai](https://github.com/0xSero/omarchy-local-ai).

## Replay one measured point

The Omarchy plugin needs the Hugging Face CLI (`hf`) on the host for this pinned runtime: its image contains Python but not `huggingface_hub`. The companion plugin change checks that prerequisite before downloading. The manual replay below uses `curl`.
