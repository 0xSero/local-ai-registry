...
|---|---|---|
| no model repository | 492 | the checkpoint is a local directory (`/model`, `/models/...`) and the post never names a Hub repository |
| no repo | 373 | same, for messages that carry numbers or a bare serve command |
| no image | 196 | the launch reads its image from an env var set elsewhere, or is an arg list without a `docker run` |
| no hardware | 605 | neither the message nor the same author's nearby messages state the rig |
| no hardware stated | 69 | same, for attachments |
| ambiguous | 26 | more than one repository or recipe could match, so nothing is claimed |
| duplicate | 5 | the same image/hardware/tensor-parallel signature is already a record |
...
| recipes added (this branch) | 81 |
| of which executable launches | 60 |
